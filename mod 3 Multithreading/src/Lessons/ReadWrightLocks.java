...
public class ReadWrightLocks {
    ...
    public static void main(String[] args) {
        ...
        IntStream.range(0, 3).forEach((i) -> new Thread(() -> {
            while (true) {
                System.out.println("Чтение: " + Arrays.toString(array.read()));
            }
        }).start());
        new Thread(() -> {
    ...
    private static class ConcurrentArray<T> {


        private final ReadWriteLock readWrightLocks = new ReentrantReadWriteLock();
        ...
        public void write(T[] values, int index) {
            readWrightLocks.writeLock().lock();
            try {

                if (items.length < values.length + index) {
                    throw new RuntimeException("Не достаточно места");
                }
                System.arraycopy(values, 0, items, index, values.length);
                Thread.sleep(random.nextInt(1000));
                System.out.println("Запись: " + Arrays.toString(items));
            } catch (InterruptedException e) {
                throw new RuntimeException(e);
            } finally {
                readWrightLocks.writeLock().unlock();
            }
        }

        public T[] read() {

            readWrightLocks.readLock().lock();
            try {
                ...
                readWrightLocks.readLock().unlock();
            }
        }
    }
}