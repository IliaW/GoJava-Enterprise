...
public class Conditions {
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> {
            while (true) {
                try{
                    System.out.println("Достаём: " + transferQueue.take());
                    Thread.sleep(1000);
                }catch (InterruptedException e){
                    e.printStackTrace();
                }
    ...
    public static class TransferQueue<T> {
        ...
        public void put(T value) throws InterruptedException {
            ...
            try {
                while (size == items.length) {
                    System.out.println("Очередь заполнена " + Thread.currentThread().getName() + " ожидает");
                    full.await();
                }
        ...
        public T take() throws InterruptedException {
            ...
            try {
                while (size == 0) {
                    System.out.println("Очередь пустая " + Thread.currentThread().getName() + " ожидает");
                    empty.await();
                }
